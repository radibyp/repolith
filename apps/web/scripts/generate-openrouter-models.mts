 ...
 *
 * Usage:
 *   bun run generate:models          # generate / update the file
 *   bun run generate:models:check    # exit 1 if the file would change (CI)
 */
...
async function main() {
	...
	if (checkOnly && previous === null) {
		console.error(
			"openrouter-models.generated.ts does not exist. Run bun run generate:models.",
		);
		process.exit(1);
	}
	...
	execFileSync("bun", ["x", "oxfmt", "--write", OUT_PATH], { stdio: "ignore" });
	...
	if (checkOnly) {
		if (previous === formatted) {
			...
			console.error(
				"Model pricing is out of date. Run bun run generate:models to update.",
			);
			process.exit(1);
		}
		return;
