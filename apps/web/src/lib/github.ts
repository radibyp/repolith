...
import {
    getCachedAuthorDossier,
    setCachedAuthorDossier,
...
function isRateLimitError(
    error: unknown,
): { resetAt: number; limit: number; used: number } | null {
    if (typeof error !== "object" || error === null) return null;
    const status = (error as { status?: number }).status;
...
function buildRepoReadmeCacheKey(
    owner: string,
    repo: string,
    ref?: string,
): string {
...
function buildTrendingReposCacheKey(
    since: string,
    perPage: number,
    language?: string,
): string {
...
function buildRepoIssuesCacheKey(
    owner: string,
    repo: string,
    state: string,
): string {
...
function buildRepoPullRequestsCacheKey(
    owner: string,
    repo: string,
    state: string,
): string {
...
function buildIssueCommentsCacheKey(
    owner: string,
    repo: string,
    issueNumber: number,
): string {
...
function buildPullRequestCacheKey(
    owner: string,
    repo: string,
    pullNumber: number,
): string {
...
function buildPullRequestFilesCacheKey(
    owner: string,
    repo: string,
    pullNumber: number,
): string {
...
function buildPullRequestCommentsCacheKey(
    owner: string,
    repo: string,
    pullNumber: number,
): string {
...
function buildPullRequestReviewsCacheKey(
    owner: string,
    repo: string,
    pullNumber: number,
): string {
...
function buildPullRequestCommitsCacheKey(
    owner: string,
    repo: string,
    pullNumber: number,
): string {
...
function buildRepoContributorsCacheKey(
    owner: string,
    repo: string,
    perPage: number,
): string {
...
function buildUserPublicReposCacheKey(
    username: string,
    perPage: number,
): string {
...
function buildRepoWorkflowRunsCacheKey(
    owner: string,
    repo: string,
    perPage: number,
): string {
...
function buildPersonRepoActivityCacheKey(
    owner: string,
    repo: string,
    username: string,
): string {
...
function buildPRBundleCacheKey(
    owner: string,
    repo: string,
    pullNumber: number,
): string {
...
async function cacheDefaultBranch(
    owner: string,
    repo: string,
    branch: string,
): Promise<void> {
...
const getGitHubAuthContext = cache(
    async (): Promise<GitHubAuthContext | null> => {
...
async function fetchRepoTagsFromGitHub(
    octokit: Octokit,
    owner: string,
    repo: string,
) {
...
async function fetchRepoReleasesFromGitHub(
    octokit: Octokit,
    owner: string,
    repo: string,
) {
...
async function fetchSearchIssuesFromGitHub(
    octokit: Octokit,
    query: string,
    perPage: number,
) {
...
async function fetchUserEventsFromGitHub(
    octokit: Octokit,
    username: string,
    perPage: number,
) {
...
async function fetchRepoContributorsFromGitHub(
    ...
    try {
        ...
        if (linkHeader) {
            ...
            if (lastMatch) {
                totalCount =
                    (parseInt(lastMatch[1], 10) - 1) * perPage + perPage;
            }
        }
...
async function fetchUserProfileFromGitHub(octokit: Octokit, username: string) {
    ...
    try {
        ...
        return {
            ...
            following: 0,
            created_at:
                (appData.created_at as string) ?? new Date().toISOString(),
            type: "Bot",
        };
    } catch {
        // all lookups failed
    ...
    try {
        ...
        return repos.map((repo, idx) =>
            languageByIndex.has(idx)
                ? {
                      ...repo,
                      language: languageByIndex.get(idx) ?? repo.language,
                  }
                : repo,
        );
    } catch {
...
async function fetchUserPublicOrgsFromGitHub(
    octokit: Octokit,
    username: string,
) {
...
async function fetchRepoWorkflowsFromGitHub(
    octokit: Octokit,
    owner: string,
    repo: string,
) {
...
async function fetchOrgMembersFromGitHub(
    octokit: Octokit,
    org: string,
    perPage: number,
) {
...
async function processGitDataSyncJob(
    ...
    switch (jobType) {
        case "user_repos": {
            ...
            const data = await fetchUserReposFromGitHub(
                authCtx.octokit,
                sort,
                perPage,
            );
        ...
        case "user_orgs": {
            ...
            const data = await fetchUserOrgsFromGitHub(
                authCtx.octokit,
                perPage,
            );
            await upsertGithubCacheEntry(
        ...
        case "org": {
            ...
            const data = await fetchOrgFromGitHub(
                authCtx.octokit,
                payload.orgName,
            );
        ...
        case "notifications": {
            ...
            const data = await fetchNotificationsFromGitHub(
                authCtx.octokit,
                perPage,
            );
            await upsertGithubCacheEntry(
        ...
        case "starred_repos": {
            ...
            const data = await fetchStarredReposFromGitHub(
                authCtx.octokit,
                perPage,
            );
            await upsertGithubCacheEntry(
    ...
    switch (jobType) {
        case "repo": {
            ...
            const repoCached = await getGithubCacheEntry(
                authCtx.userId,
                repoKey,
            );
        ...
        case "repo_tags": {
            const data = await fetchRepoTagsFromGitHub(
                authCtx.octokit,
                owner,
                repo,
            );
        ...
        case "pull_request": {
            ...
            const prKey = buildPullRequestCacheKey(
                owner,
                repo,
                payload.pullNumber,
            );
        ...
        case "pull_request_comments": {
            ...
            await upsertCacheWithShared(
                authCtx.userId,
                buildPullRequestCommentsCacheKey(
                    owner,
                    repo,
                    payload.pullNumber,
                ),
        ...
        case "pull_request_reviews": {
            ...
            await upsertCacheWithShared(
                authCtx.userId,
                buildPullRequestReviewsCacheKey(
                    owner,
                    repo,
                    payload.pullNumber,
                ),
        ...
        case "pull_request_commits": {
            ...
            await upsertCacheWithShared(
                authCtx.userId,
                buildPullRequestCommitsCacheKey(
                    owner,
                    repo,
                    payload.pullNumber,
                ),
...
async function drainGitDataSyncQueue(authCtx: GitHubAuthContext, limit = 4) {
    const jobs = await claimDueGithubSyncJobs<GitDataSyncJobPayload>(
        authCtx.userId,
        limit,
    );
...
async function enqueueGitDataSync(
    ...
    if (isShareableCacheType(jobType)) {
        ...
        if (
            shared &&
            Date.now() - new Date(shared.syncedAt).getTime() < 2 * 60 * 1000
        ) {
...
export async function getUserRepos(sort: RepoSort = "updated", perPage = 30) {
    ...
    return readLocalFirstGitData({
        ...
        fetchRemote: (octokit) =>
            fetchUserReposFromGitHub(octokit, sort, perPage),
    });
}
...
export async function getNotifications(perPage = 20) {
    ...
    return readLocalFirstGitData({
        ...
        jobPayload: { perPage },
        fetchRemote: (octokit) =>
            fetchNotificationsFromGitHub(octokit, perPage),
    });
}
...
export async function searchIssues(query: string, perPage = 20) {
    ...
    return readLocalFirstGitData({
        ...
        fetchRemote: (octokit) =>
            fetchSearchIssuesFromGitHub(octokit, query, perPage),
    });
}
...
export async function getUserEvents(username: string, perPage = 30) {
    ...
    return readLocalFirstGitData({
        ...
        fetchRemote: (octokit) =>
            fetchUserEventsFromGitHub(octokit, username, perPage),
    });
}
...
export async function getRepoContents(
    ...
    const cacheKey = buildRepoContentsCacheKey(
        owner,
        repo,
        path,
        normalizedRef,
...
export async function getRepoBranches(owner: string, repo: string) {
    ...
    return readLocalFirstGitData({
        ...
        jobPayload: { owner, repo },
        fetchRemote: (octokit) =>
            fetchRepoBranchesFromGitHub(octokit, owner, repo),
    });
}
...
export async function getRepoReleases(owner: string, repo: string) {
    ...
    return readLocalFirstGitData({
        ...
        jobPayload: { owner, repo },
        fetchRemote: (octokit) =>
            fetchRepoReleasesFromGitHub(octokit, owner, repo),
    });
}

export async function getRepoReleasesPage(
    owner: string,
    repo: string,
    page: number,
) {
...
export async function getRepoTagsPage(
    owner: string,
    repo: string,
    page: number,
) {
...
export async function getRepoReleaseByTag(
    owner: string,
    repo: string,
    tag: string,
) {
    ...
    try {
        ...
        return nodes.map((thread: Record<string, unknown>) => ({
            ...
            resolvedBy: thread.resolvedBy
                ...
                : null,
            comments: (
                (thread.comments as { nodes?: Record<string, unknown>[] })
                    ?.nodes ?? []
...
function transformGraphQLPRBundle(node: GQLPRNode): PRBundleData {
    ...
    const stateEvents: PRBundleData["stateEvents"] = (
...
export async function getPullRequestBundle(
    ...
    return readLocalFirstGitData({
        ...
        jobPayload: { owner, repo, pullNumber },
        fetchRemote: () =>
            fetchPRBundleFromGitHub(authCtx!.token, owner, repo, pullNumber),
    });
}
...
export async function getIssue(
    ...
    return readLocalFirstGitData({
        ...
        fetchRemote: (octokit) =>
            fetchIssueFromGitHub(octokit, owner, repo, issueNumber),
    });
}
...
export async function getRepoIssues(
    ...
    return readLocalFirstGitData({
        ...
        fetchRemote: (octokit) =>
            fetchRepoIssuesFromGitHub(octokit, owner, repo, state),
    });
}
...
function mapGQLDiscussionComment(
    node: GQLDiscussionCommentNode,
): DiscussionComment {
    return {
        id: node.id,
...
export async function fetchMoreDiscussions(
    ...
    const authCtx = await getGitHubAuthContext();
    if (!authCtx)
        return { discussions: [], hasNextPage: false, endCursor: null };
    const data = await fetchRepoDiscussionsPageGraphQL(
        authCtx.token,
        owner,
        repo,
        cursor,
...
export async function getDiscussion(
    ...
    const authCtx = await getGitHubAuthContext();
    if (!authCtx) return null;
    const result = await fetchDiscussionDetailGraphQL(
        authCtx.token,
        owner,
        repo,
        number,
...
export async function toggleDiscussionUpvote(
    ...
    const key = hasUpvoted
        ? "removeUpvoteFromDiscussion"
        : "addUpvoteToDiscussion";
...
export async function invalidateRepoDiscussionsCache(
    owner: string,
    repo: string,
) {
    const authCtx = await getGitHubAuthContext();
...
function mapGraphQLIssueNode(node: GQLIssueNode): RepoIssueNode {
    ...
    return {
        ...
        state_reason: node.stateReason
            ? (stateReasonMap[node.stateReason] ?? null)
            : null,
        ...
        user: author
            ? { login: author.login, avatar_url: author.avatarUrl }
            : null,
        labels: (node.labels?.nodes ?? []).map((l) => ({
            name: l.name,
            color: l.color,
...
export async function invalidateAllPRBundlesForRepo(
    owner: string,
    repo: string,
) {
    const authCtx = await getGitHubAuthContext();
...
export async function invalidatePullRequestCache(
    ...
    await deleteGithubCacheByPrefix(
        authCtx.userId,
        `pr_bundle:${key}:${pullNumber}`,
    );
    await deleteGithubCacheByPrefix(
        authCtx.userId,
        `pull_request:${key}:${pullNumber}`,
    );
    await deleteGithubCacheByPrefix(
        authCtx.userId,
    ...
    await deleteGithubCacheByPrefix(
        authCtx.userId,
        `pull_request_files:${key}:${pullNumber}`,
    );
    await deleteGithubCacheByPrefix(
        authCtx.userId,
        `repo_pull_requests:${key}`,
    );
...
export async function invalidateIssueCache(
    ...
    await deleteGithubCacheByPrefix(
        authCtx.userId,
        `issue_comments:${key}:${issueNumber}`,
    );
...
function mapGraphQLPRNode(pr: Record<string, unknown>) {
    ...
    const assignees = (
        (pr.assignees as { nodes: { login: string; avatarUrl: string }[] })
            ?.nodes ?? []
    ...
    return {
        ...
        review_comments:
            (pr.reviewThreads as { totalCount: number })?.totalCount ?? 0,
        user: author
            ? { login: author.login, avatar_url: author.avatarUrl }
            : null,
        labels,
...
function normalizeCheckConclusion(
    ...
    if (status === "completed") {
        ...
        if (conclusion === "action_required" || conclusion === "stale")
            return "pending";
        return "failure";
    }
    return "pending";
...
async function fetchCheckStatusForRef(
    ...
    > | null = null;
    let checkRuns: Awaited<
        ReturnType<typeof octokit.checks.listForRef>
    > | null = null;

    try {
        commitStatuses = await octokit.repos.getCombinedStatusForRef({
            owner,
            repo,
            ref,
        });
    ...
    try {
        checkRuns = await octokit.checks.listForRef({
            owner,
            repo,
            ref,
            per_page: 100,
...
export async function enrichPRsWithCheckStatus(
    ...
    const results = await Promise.allSettled(
        prs.map((pr) =>
            fetchCheckStatusForRef(octokit, owner, repo, pr.head.sha).then(
                (cs) => ({
                    number: pr.number,
                    checkStatus: cs,
                }),
...
export async function batchFetchCheckStatuses(
    ...
    try {
        ...
        for (let i = 0; i < prs.length; i++) {
            ...
            for (const ctx of contexts) {
                if (ctx.__typename === "CheckRun") {
                    const runIdMatch = ctx.detailsUrl?.match(
                        /\/actions\/runs\/(\d+)/,
                    );
                    checks.push({
                        name: ctx.name,
        ...
        if (Object.keys(result).length > 0) {
            redis
                .set(rKey, result, { ex: CHECK_STATUS_REDIS_TTL })
                .catch(() => {});
        }
...
export async function prefetchPRData(
    ...
    try {
        ...
        const prPrefetch = cached
            ? Promise.resolve()
            : getRepoPullRequestsWithStats(owner, repo, "open").then(
                  ({ prs }) =>
                      prs.length > 0
...
function extractRepoSecurityFeatures(
    repoData: unknown,
): RepoSecurityFeatures | null {
    ...
    const [repoResult, reports, dependabot, secretScanning] = await Promise.all(
        [
            ...
            octokit.securityAdvisories
                ...
                .then((result) => ({
                    alerts: result.data.map((advisory) =>
                        mapSecurityReport(advisory),
                    ),
                    error: null as string | null,
                }))
                .catch((error: unknown) => ({
...
export async function getRepoWorkflows(owner: string, repo: string) {
    ...
    return readLocalFirstGitData({
        ...
        jobPayload: { owner, repo },
        fetchRemote: (octokit) =>
            fetchRepoWorkflowsFromGitHub(octokit, owner, repo),
    });
}
...
export async function getUserPublicOrgs(username: string) {
    ...
    return readLocalFirstGitData({
        ...
        jobPayload: { username },
        fetchRemote: (octokit) =>
            fetchUserPublicOrgsFromGitHub(octokit, username),
    });
}
...
export async function getOrgMembers(org: string, perPage = 100) {
    ...
    return readLocalFirstGitData({
        ...
        fetchRemote: (octokit) =>
            fetchOrgMembersFromGitHub(octokit, org, perPage),
    });
}
...
export async function getRepoContributorStats(
    ...
    try {
        ...
        let response = await octokit.repos.getContributorsStats({
            owner,
            repo,
        });
        if (response.status === 202) {
            await new Promise((r) => setTimeout(r, 2000));
            response = await octokit.repos.getContributorsStats({
                owner,
                repo,
            });
        }
...
export async function getCommitActivity(
    ...
    try {
        ...
        if (!Array.isArray(response.data)) return [];
        return (
            response.data as { total: number; week: number; days: number[] }[]
        ).map((w) => ({
            total: w.total ?? 0,
            week: w.week ?? 0,
            days: w.days ?? [],
...
async function fetchRepoPageDataGraphQL(
    ...
    if (!r) {
        console.warn(
            `[fetchRepoPageDataGraphQL] Repository not found: ${owner}/${repo}`,
        );
        return null;
    }
...
export const getRepoPageData = cache(
    async (owner: string, repo: string): Promise<RepoPageDataResult> => {
        ...
        const { getCachedRepoPageData } =
            await import("@/lib/repo-data-cache-vc");
...
export async function fetchAndCacheRepoPageData(
    ...
    try {
        const result = await fetchRepoPageDataGraphQL(
            authCtx.token,
            owner,
            repo,
        );
        ...
        console.error(
            `[fetchAndCacheRepoPageData] Failed for ${owner}/${repo}:`,
            error,
        );
        const message =
...
export async function getRepoOverviewData(
    ...
    if (result.success)
        return {
            navCounts: result.data.navCounts,
...
async function fetchPersonRepoActivityFromGitHub(
    ...
    const [commitsResult, prsResult, issuesResult, reviewsResult] =
