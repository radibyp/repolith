...
export function PRDiffViewer({
    ...
    const [wordWrap, setWordWrapState] = useState(
        () => getDiffPreferences().wordWrap,
    );
    const [splitView, setSplitViewState] = useState(
        () => getDiffPreferences().splitView,
    );
    ...
    useEffect(() => {
        ...
        return () =>
            window.removeEventListener("ghost:navigate-to-file", handler);
    }, [files]);
                            ...
                            {sidebarMode === "files" ? (
                                <>
                                    {files.map((file, i) => {
                                        ...
                                        const isViewed = viewedFiles.has(
                                            file.filename,
                                        );
                                        const fileThreads = threadsByFile.get(
                                            file.filename,
                                        );
...
function SingleFileDiff({
    ...
    useEffect(() => {
        ...
        const row = diffContainerRef.current.querySelector(
    ...
    const [expandedLines, setExpandedLines] = useState<Map<number, string[]>>(
        new Map(),
    );
    const [fileContent, setFileContent] = useState<string[] | null>(null);
    const [fullFileTokens, setFullFileTokens] = useState<
        SyntaxToken[][] | null
    >(null);
    ...
    const fetchFileContent = useCallback(
        async (withHighlight = false): Promise<string[] | null> => {
            if (fileContent && (!withHighlight || fullFileTokens))
                return fileContent;
    ...
    const handleCommitEdit = useCallback(
        async (message: string) => {
            if (!owner || !repo || !pullNumber || !headBranch || !editSha)
                return;
        ...
        ? (() => {
              ...
              const matchingLines = lines.filter((l) => {
                  ...
                  if (isLeft) {
                      ...
                      const ln = l.oldLineNumber;
                      return (
                          ln !== undefined && ln >= startLine && ln <= endLine
                      );
                  } else {
                      if (l.type === "remove") return false;
                      const ln = l.newLineNumber;
        ...
        selectingFrom && hoverLine !== null
            ...
            : commentRange
              ? {
                    ...
                    end: Math.max(commentRange.startLine, commentRange.endLine),
                    ...
                    <>
                        ...
                        {editView === "changes" && (
                            <div className="flex-1 overflow-auto">
                                {(() => {
                                    const diffBase =
                                        baseEditContent ?? originalEditContent;
                                    ...
                                    const diffEntries = computeLineDiff(
                                        diffBase,
                                        editContent,
                                    );
                                    return (
                    ...
                    {canComment &&
                        ...
                        line.type !== "header" && (
                            <button
                                ...
                                onClick={(e) =>
                                    onLineClick(lineNum, side, e.shiftKey)
                                }
            ...
            >
                ...
                <tbody>
                    {splitRows.map((row, i) => {
                        if (row.type === "header") {
                            const funcMatch =
                                row.headerContent?.match(/@@ .+? @@\s*(.*)/);
                            const funcName = funcMatch?.[1];
                            const expandedContent =
                                row.hunkIndex !== undefined
                                    ? expandedLines.get(row.hunkIndex)
                                    : undefined;
                            ...
                            if (expandedContent && hunkIdx !== undefined) {
                                ...
                                if (currentHunk) {
                                    const prevHunk = hunkInfos
                                        .filter((h) => h.index < hunkIdx)
                                        .pop();
                                    expandStartLine = prevHunk
                                        ? prevHunk.endNewLine + 1
                                        : 1;
                                }
                            }
                        ...
                        return (
                            <React.Fragment key={`p-${i}`}>
                                <tr
                                    ...
                                    onMouseEnter={() => {
                                        if (leftLineNum !== undefined)
                                            onLineHover(leftLineNum);
                                        if (rightLineNum !== undefined)
                                            onLineHover(rightLineNum);
                                    }}
                                >
                                    {renderHalf(
                                        row.left,
                                ...
                                {/* Comment form */}
                                {(leftIsCommentForm || rightIsCommentForm) &&
                                    commentRange && (
                                        <tr>
                                            {commentRange.side === "LEFT" ? (
                                                <>
                                                    <td
                                                        colSpan={3}
                                                        className="p-0 align-top"
            ...
            <tbody>
                {mergedRows.map((row, i) => {
                    ...
                    return (
                        <tr
                            key={`ln-${i}`}
                            className={isAdd ? "diff-add-row" : undefined}
                        >
                            <td
                                className={cn(
                                    "w-[3px] p-0 sticky left-0 z-[1]",
