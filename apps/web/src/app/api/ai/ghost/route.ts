...
const GHOST_MODELS = {
    ...
    mergeConflict: process.env.GHOST_MERGE_MODEL || "qwen/qwen3.5-122b-a10b",
} as const;
    ...
    return {
        getMergeConflictInfo: tool({
            ...
            inputSchema: z.object({
                ...
                baseBranch: z
                    .string()
                    .describe("Target/base branch (e.g. 'main')"),
                headBranch: z
                    .string()
            ...
            execute: async ({ owner, repo, baseBranch, headBranch }) => {
                try {
                    ...
                    if (files.length === 0) {
                        return {
                            message:
                                "No file differences found between branches.",
                        };
                    }
                                ...
                                async (f: {
                                    ...
                                    try {
                                        ...
                                        if (
                                            ...
                                            result.baseContent = Buffer.from(
                                                (
                                                    baseData as {
                                                        content: string;
                                                    }
                                    ...
                                    try {
                                        ...
                                        if (
                                            ...
                                            result.headContent = Buffer.from(
                                                (
                                                    headData as {
                                                        content: string;
                                                    }
        ...
        commitMergeResolution: tool({
            ...
            inputSchema: z.object({
                ...
                baseBranch: z
                    .string()
                    .describe("Target/base branch to merge in"),
                resolvedFiles: z
                    .array(
                        z.object({
                            path: z
            ...
            execute: async ({ command, cwd, timeout }) => {
                ...
                try {
                    ...
                    if (result.exitCode !== 0) {
                        return {
                            error:
                                stdout.trim() || `exit code ${result.exitCode}`,
...
export async function POST(req: Request) {
    ...
    const generalTools = getGeneralTools(
        octokit,
        pageContext,
        userId ?? undefined,
    );
    const codeEditTools = getCodeEditTools(octokit, commitAuthor);
    const sandboxTools = githubToken
        ? getSandboxTools(octokit, githubToken, commitAuthor)
        : {};
    ...
    const recalledMemories = userId
        ? await recallMemoriesForContext(userId, messages)
        : "";
    ...
    if (resolvedPrContext) {
        ...
        const repoMatch = pageContext?.pathname?.match(
            /^\/repos\/([^/]+)\/([^/]+)/,
        );
        if (repoMatch) {
